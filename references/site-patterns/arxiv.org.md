...
- 限速：连续请求间隔应 ≥ 3s；一旦触发 429（Varnish 网关返回 14 字节空 body 或 503），cooldown 长达 **60–90s**，10–30s 内重试无效。停止重试，等满 90s 再发，或改换 query 形态绕开缓存桶（更新于 2026-04-07）
- 搜索结果按相关度排序时，`sortBy=relevance` 效果不稳定，`submittedDate` 更可预期
- 时间窗检索优先用 `submittedDate:[YYYYMMDDHHMM TO YYYYMMDDHHMM]` 范围语法（如 `submittedDate:[202603070000 TO 202604072359]`），比"主题词 + 客户端按 `<published>` 过滤"更可靠——后者只返回最相关的若干条，命中率低（发现于 2026-04-07）
