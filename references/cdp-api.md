...
## ⚠️ chrome-devtools MCP `evaluate_script` 的 `args` 陷阱

`mcp__chrome-devtools__evaluate_script` 的 `args` 字段**不是**通用 JS 参数 — 它只接受 element uid 字符串（来自 take_snapshot），用来把页面元素引用注入到回调函数。

**错误**：把检索式/字符串/数字塞进 `args`：
```
args: ["SU=('LLM') AND SU=('中医')"]   # ❌ 报 "Element uid ... not found"
```

**正确**：所有需要的字面量直接写到函数体里：
```
function: () => { const expression = "SU=('LLM') AND SU=('中医')"; ... }
```

需要传 element 引用时再用 `args: [uid_from_snapshot]`，函数签名 `(el) => el.click()`。
...
- 启动：`CDP_PROXY_PORT=3456 node ~/.claude/skills/paper-search/scripts/cdp-proxy.mjs &`
...
| `WebSocket 未连接` | Proxy 启动后 Chrome 断连 | 重启 `scripts/cdp-proxy.mjs` 重连 |
